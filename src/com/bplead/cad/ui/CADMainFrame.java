...
public class CADMainFrame extends AbstractFrame implements Callback {
	...
	/**
	 * TODO checkin checkout undocheckout
	 * 
	...
	public void initialize() {
		...
		setJMenuBar(
				toolkit.getStandardMenuBar(new CheckinActionListenner(this), new CheckoutAndDownloadActionListenner()));
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (StringUtils.isEmpty(checkComfirmClient)) {
				...
				if (StringUtils.isEmpty(checkComfirmServer)) {
					...
					int n = JOptionPane.showConfirmDialog(null, checkComfirmServer, "图纸代号校验",
				...
				for (int i = 0; i < documentL.size(); i++) {
					...
					for (Attachment attachment : attachments) {
						...
						if (logger.isDebugEnabled()) {
							logger.debug("file exists is -> " + file.exists() + " attachment.getAbsolutePath () is -> "
									+ attachment.getAbsolutePath());
						}
						// build new filename: oldfilename + uuid
						String newFileName = ClientUtils.buildNewFileName(file);
						if (logger.isDebugEnabled()) {
							logger.debug("客户端文件路径 is -> " + attachment.getAbsolutePath() + " ftp到服务器端后新的文件名 is -> "
									+ newFileName);
						}
						...
						if (upload) {
							publish(new PopProgress.PromptProgress(
									"上传客户端文件" + attachment.getAbsolutePath() + "到ftp服务器成功.", 10));
						} else {
							publish(new PopProgress.PromptProgress(
									"上传客户端文件" + attachment.getAbsolutePath() + "到ftp服务器失败.", 20));
						}
					}
				}
			}
	...
	public class ExportDetailActionListenner implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				JOptionPane.showMessageDialog(null, getResourceMap().getString(PROMPT_SUCCESSED) + ",请查看文件" + filePath,
						getResourceMap().getString(PROMPT_TITLE), JOptionPane.INFORMATION_MESSAGE);
